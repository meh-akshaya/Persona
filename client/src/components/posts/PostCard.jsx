    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 mb-3.5 min-w-0">
        <div className="flex items-center gap-2.5 min-w-0 flex-1 flex-wrap">
          ...
          <span className="text-[13px] font-semibold text-[#F2F2F2] truncate max-w-[140px] sm:max-w-[200px]">
            {post.author?.personaName}
          </span>

          <span className="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded-[4px] bg-[#0D0D0F] text-[#9A9A9F] border border-[#25252A] flex items-center gap-1">
            Trust <span className="text-[#F5B800] font-bold">{post.author?.trustScore ?? 0}</span>
          </span>

          <span className="text-[#6F7076] text-xs shrink-0">·</span>

          <CleanLink
            to={`/c/${post.community?.slug}`}
            onClick={(e) => e.stopPropagation()}
            className="text-xs text-[#9A9A9F] hover:text-[#F2F2F2] font-medium transition-colors truncate max-w-[120px] sm:max-w-[180px] shrink-0 px-2 py-0.5 rounded-[4px] bg-[#0D0D0F] border border-[#25252A]"
          >
            {post.community?.name}
          </CleanLink>
        </div>

        <span className="text-[11px] text-[#6F7076] shrink-0 ml-auto text-right whitespace-nowrap self-start pt-0.5 pr-0.5">
      ...
      <div
        className="mt-3.5 mb-4 select-none min-h-[24px]"
        onCopy={handleCopyAttempt}
        onCut={handleCopyAttempt}
        onDragStart={(e) => e.preventDefault()}
      >
        ...
        {isLong && (
          <button
            ...
            className="mt-2 text-xs font-semibold text-[#F5B800] hover:text-[#ffd24d] transition-colors cursor-pointer inline-flex items-center gap-1 focus:outline-none"
          >
        ...
        {post.hasPrivacyLeak && (
          <CleanLink to={`/post/${post.id}`} className="block">
            <div
              ...
              className="mt-3.5 p-3.5 rounded-[6px] border text-xs flex items-center gap-2.5 font-medium"
            >
      ...
      {/* Action Toolbar — Dedicated Action Row with Top Separator Border */}
      <div className="pt-3.5 border-t border-[#25252A]/60 flex items-center justify-between gap-3 mt-4 text-xs text-[#9A9A9F] flex-wrap sm:flex-nowrap min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          ...
          <button
            ...
            className={`h-8 px-2.5 sm:px-3 rounded-[6px] text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer border shrink-0 ${activeReaction === 'HELPFUL'
                ? 'bg-[#F5B800]/15 text-[#F5B800] border-[#F5B800]/40 font-bold'
                : 'bg-[#0D0D0F] text-[#9A9A9F] border-[#25252A] hover:text-[#F2F2F2] hover:bg-[#151518]'
              }`}
          >
            <svg className="w-3.5 h-3.5 fill-none stroke-current shrink-0" strokeWidth="1.75" viewBox="0 0 24 24">
          ...
          <button
            ...
            className={`h-8 px-2.5 sm:px-3 rounded-[6px] text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer border shrink-0 ${activeReaction === 'INSIGHTFUL'
                ? 'bg-[#F5B800]/15 text-[#F5B800] border-[#F5B800]/40 font-bold'
                : 'bg-[#0D0D0F] text-[#9A9A9F] border-[#25252A] hover:text-[#F2F2F2] hover:bg-[#151518]'
              }`}
          >
            ...
            <span className="whitespace-nowrap">Insightful</span>
          </button>
          ...
          <button
            ...
            className={`h-8 px-2.5 sm:px-3 rounded-[6px] text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer border shrink-0 ${activeReaction === 'SUPPORTIVE'
                ? 'bg-[#F5B800]/15 text-[#F5B800] border-[#F5B800]/40 font-bold'
                : 'bg-[#0D0D0F] text-[#9A9A9F] border-[#25252A] hover:text-[#F2F2F2] hover:bg-[#151518]'
              }`}
          >
          ...
          {reactionsCount > 0 && (
            <span className="text-[11px] font-semibold text-[#6F7076] ml-1 shrink-0">
              {reactionsCount}
            </span>
          )}
        </div>

        {/* Right Comments Group — Pushed to Far Right */}
        <CleanLink
          to={`/post/${post.id}`}
          className="ml-auto shrink-0 flex items-center h-8 gap-1.5 px-3 rounded-[6px] bg-[#0D0D0F] border border-[#25252A] text-xs text-[#9A9A9F] hover:text-[#F2F2F2] hover:bg-[#151518] font-medium transition-colors whitespace-nowrap"
        >
          <svg className="w-3.5 h-3.5 fill-none stroke-current shrink-0" strokeWidth="1.75" viewBox="0 0 24 24">
